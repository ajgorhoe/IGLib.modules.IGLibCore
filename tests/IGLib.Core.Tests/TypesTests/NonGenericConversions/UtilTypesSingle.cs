...
namespace IGLib.Types.Single.Extensions
{
    ...
    public static class UtilTypesSingle
    {
        ...
        {
            if (value is null)
                throw new ArgumentNullException(nameof(value), $"{nameof(UtilTypesSingle)}.{nameof(ToInt)}: Value is null.");
            ...
            if (value is string s)
            {
                ...
                throw new FormatException(
                    $"{nameof(UtilTypesSingle)}.{nameof(ToInt)}: Cannot parse string '{s}' to int using {((CultureInfo)formatProvider).Name} culture.");
            }
            ...
            throw new InvalidOperationException(
                $"{nameof(UtilTypesSingle)}.{nameof(ToInt)}: Value {value} of type {value.GetType().Name} cannot be converted to int{(precise ? " precisely" : "")}.");
        }
