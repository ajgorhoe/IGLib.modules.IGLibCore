...
namespace IGLib.Types.Tests
{
    ...
    public class UtilTypesTests : TestBase<UtilTypesTests>
    {
        ...
        {
            ConvertToTypeTestCommon<int>(true, converted, precise, expectedResult, failureExpected, comment);
        }
        ...
        protected void IsConvertibleTo_Generic_WorksCorrectlyFor_Int(object converted, bool precise, int expectedResult,
            bool failureExpected = false, string comment = null)
        {
            ConvertToTypeTestCommon<int>(false, converted, precise, expectedResult, failureExpected, comment);
        }
        ...
        protected void ConvertToTypeTestCommon<TargetType>(bool testConversion, object converted, bool precise, int expectedResult,
            bool failureExpected = false, string comment = null)
            where TargetType : IConvertible
        {
            ...
            try
            {
                ...
                if (!testConversion)
                {
                    isConvertible.Should().Be(!failureExpected, because: $"tesult of {nameof(UtilTypes.IsConvertibleTo)
                        } is expected to be {!failureExpected}");
                }
            }
            catch (Exception)
            {
