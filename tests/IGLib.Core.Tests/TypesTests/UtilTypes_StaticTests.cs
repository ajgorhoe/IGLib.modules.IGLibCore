...
namespace IGLib.Types.Tests
{
    ...
    public class UtilTypesTests : TestBase<UtilTypesTests>
    {
        ...
        //    try
        //    {
        //        isConvertible = UtilTypes.IsConvertibleToInt(converted, precise: precise);
        //        if (!testConversion)
        //        {
        ...
        [InlineData((char)2836, false, 2836, false, "conversion of non-ASCII character to int")]
        [Obsolete]
        protected void ConvertToType_WorksCorrectly(object converted, bool precise, int expectedResult,
            bool failureExpected = false, string comment = null)
        {
            ConvertToTypeTestCommon(true, converted, precise, expectedResult, failureExpected, comment);
        }
        ...
        [InlineData((char)2836, false, 2836, false, "conversion of non-ASCII character to int")]
        [Obsolete]
        protected void IsConvertibleToType_WorksCorrectly(object converted, bool precise, int expectedResult,
            bool failureExpected = false, string comment = null)
        {
            ConvertToTypeTestCommon(false, converted, precise, expectedResult, failureExpected, comment);
        }
        ...
        /// <param name="comment">Optional comment, which will be output and helps in checking and interpreting the results.</param>
        [Obsolete]
        protected void ConvertToTypeTestCommon(bool testConversion, object converted, bool precise, int expectedResult,
            bool failureExpected = false, string comment = null)
        {
            if (testConversion)
            {
            ...
            try
            {
                int result = UtilTypes.ConvertTo<int>(converted, precise: precise);
                Console.WriteLine($"Result of conversion: {result}, expected: {expectedResult}");
                if (testConversion)
                {
                    result.Should().Be(expectedResult);
            ...
            try
            {
                isConvertible = UtilTypes.IsConvertibleTo<int>(converted, precise: precise);
                if (!testConversion)
                {
                    isConvertible.Should().Be(!failureExpected, because: "Result of IsConvertible should be as expected.");
                }
